...
/**
 * App ID Meta ialah nombor sahaja. Medan ni kerap terisi dengan benda lain —
 * selalunya base URL atau key pembekal AI — dan Meta cuma balas
 * "Invalid client_id" tanpa beritahu medan mana yang salah.
 */
export function validateAppId(appId) {
  const id = String(appId ?? "").trim();
  if (!id) throw new Error("App ID belum diisi.");
  if (/^\d+$/.test(id)) return id;

  const petunjuk = "App ID betul ada di developers.facebook.com/apps → pilih app kau → App settings → Basic → App ID.";
  if (/^https?:\/\//i.test(id) || /\//.test(id)) {
    throw new Error(`"${id}" ialah alamat web, bukan App ID. Base URL pembekal AI masuk dalam kad "Enjin ayat" `
      + `di halaman utama, bukan kad Threads ni. ${petunjuk}`);
  }
  if (/^(sk-|asai_|hf_|or-|Bearer\b)/i.test(id)) {
    throw new Error(`"${id.slice(0, 12)}…" nampak macam API key AI, bukan App ID. Key tu masuk dalam kad "Enjin ayat" `
      + `di halaman utama. ${petunjuk}`);
  }
  throw new Error(`App ID Meta ialah nombor sahaja (lebih kurang 15-16 digit) — "${id}" ada aksara bukan nombor. ${petunjuk}`);
}
...
export function extractCode(input) {
  ...
  const err = raw.match(/[?&#]error_message=([^&\s#]+)/)
    || raw.match(/[?&#]error_description=([^&\s#]+)/)
    || raw.match(/[?&#]error=([^&\s#]+)/);
  if (err) {
    const pesan = decodeURIComponent(err[1]).replace(/\+/g, " ");
    if (/unknown error/i.test(pesan)) {
      throw new Error("Meta bagi ralat umum (kod 1). Empat sebab biasa, ikut susunan: "
        + "(1) akaun Threads yang kau approve tu belum terima jemputan Threads Tester — semak Settings → More settings → Website permissions → Invites; "
        + "(2) akaun tu bukan public; "
        + "(3) kau belum log masuk Threads dalam tetingkap tu — buka threads.com dan log masuk dulu, baru buka pautan kebenaran; "
        + "(4) kau buka pautan dalam tetingkap InPrivate atau Incognito — sekatan cookie pihak ketiga di situ kerap mematahkan OAuth Meta; guna profil browser berasingan atau log keluar akaun pertama dalam tetingkap biasa; "
        + "(5) kadang ia memang sekejap sahaja — cuba lagi dalam seminit.");
    }
    if (/URL Blocked|not whitelisted/i.test(pesan)) {
      throw new Error("Meta kata redirect URI tu belum didaftarkan. Buka use case Threads → Settings, "
        + "isi ketiga-tiga medan (Redirect Callback URLs, Deauthorize callback URL, Data deletion request URL) "
        + "dengan alamat yang sama, klik cadangan yang muncul di bawah medan, tekan Save, lepas tu muat semula "
        + "halaman untuk sahkan ia betul-betul tersimpan.");
    }
    throw new Error(`Meta tolak kebenaran: ${pesan}`);
  }
