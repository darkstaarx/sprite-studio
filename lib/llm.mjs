...
export const ANGLES = {
  ...
  "lawak":          { label: "Cerita lawak", shape: "Babak yang memang kelakar → hiperbola ringan yang orang faham bukan fakta → produk muncul sebagai penyelesaian tak sengaja → tutup dengan ayat yang buat orang senyum, bukan CTA" },
...
export async function generate({ settings, brief, platforms, count }) {
  ...
  const semak = posts.map((p, index) => ({ index, isu: lintPost(p, { sebutHarga: brief.sebutHarga === true, panjang: brief.panjang }) }));
  ...
  if (arahan) {
    try {
      ...
      if (res2.ok) {
        ...
        if (baru.length) {
          const isuBaru = baru.reduce((n, p) => n + lintPost(p, { sebutHarga: brief.sebutHarga === true, panjang: brief.panjang }).length, 0);
  ...
  const lint = posts.map(p => lintPost(p, { sebutHarga: brief.sebutHarga === true, panjang: brief.panjang }));
