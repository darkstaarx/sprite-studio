...
// Bahasa baku: betul dalam karangan, tapi tiada siapa taip begini dalam telefon.
const BAKU = [
  { re: /\btetapi\b/i, ganti: "tapi" },
  { re: /\bkerana\b/i, ganti: "sebab" },
  { re: /\b(sangat|amat|amatlah)\b/i, ganti: "gila / memang" },
  { re: /\b(adalah|ialah|merupakan)\b/i, ganti: "buang terus" },
  { re: /\b(namun|justeru|walhal|tatkala|seterusnya)\b/i, ganti: "tapi / jadi" },
  { re: /\b(oleh itu|di samping itu|walau bagaimanapun|dengan itu)\b/i, ganti: "jadi" },
  { re: /\b(tersebut|amatlah|begitu juga)\b/i, ganti: "buang terus" },
  { re: /\banda\b/i, ganti: "kau / korang" },
];
const SENGKANG = /[—–]/;
...
export function lintPost(post, { sebutHarga = false, panjang = "sederhana", had = 500, angle = "" } = {}) {
  ...
  // Hook = ayat pertama, bukan perenggan pertama. Orang tulis satu blok tanpa
  // baris kosong, jadi memotong pada "\n" sahaja menyalahkan post yang elok.
  const perengganPertama = teks.split("\n")[0].trim();
  const barisPertama = (perengganPertama.match(/^[^.!?\n]+[.!?]?/) || [perengganPertama])[0].trim();
  ...
  for (const b of BAKU) {
    const jumpa = teks.match(b.re);
    if (jumpa) isu.push({ kod: "bahasa-baku", pesan: `"${jumpa[0]}" terlalu baku untuk Threads — guna ${b.ganti}.` });
  }
  if (SENGKANG.test(teks)) isu.push({ kod: "sengkang", pesan: "Ada sengkang panjang (—). Tiada siapa taip benda tu atas telefon — guna koma atau ayat baharu." });
