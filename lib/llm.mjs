...
import { explain } from "./llm-errors.mjs";
import { lintPost, arahanBetulkan } from "./post-lint.mjs";
...
export const ANGLES = {
  ...
  "circle":         { label: "Story circle", shape: "BIASA → MAHU → KELUAR → CARI (butiran konkrit) → JUMPA → HARGA yang kau bayar → BALIK → BERUBAH. Beat HARGA wajib ada." },
  "hottake":        { label: "Hot take", shape: "Claim berani dalam baris pertama → kenapa orang lain salah faham → bukti atau pengalaman kau → jemput orang bantah" },
  "bina":           { label: "Bina dalam terang", shape: "Apa aku tengah cuba sekarang → apa yang tak jadi minggu ni → apa aku belajar → apa aku cuba seterusnya" },
  "lawak":          { label: "Cerita lawak", shape: "Satu babak sebenar dengan kau sebagai bahan (bila, kat mana, siapa nampak) → apa yang jadi, tanpa terangkan produk → tutup dengan akibat kecil yang masih berlaku sampai sekarang. Jangan buka dengan pepatah, jangan letak teka-teki, jangan terangkan jenaka sendiri. Taip macam hantar mesej dalam group: partikel (la, plak, oi, haa), ulang perkataan, tanda seru, ketawa ditulis." },
...
export function buildBrief(brief, platforms, count) {
  ...
  const sebutHarga = brief.sebutHarga === true;
  if (brief.harga) {
    lines.push(sebutHarga
      ? `Harga/promo: ${brief.harga} — brief ini MEMBENARKAN harga disebut dalam post.`
      : `Harga (untuk pengetahuan kau sahaja, JANGAN tulis dalam post): ${brief.harga}`);
  }
  if (!sebutHarga) {
    lines.push("JANGAN sebut atau anggar harga dalam post. Biar pembaca klik link untuk tahu sendiri. "
      + "Elak juga 'murah', 'berbaloi', 'bawah RM__'.");
  }
  const PANJANG = { pendek: "100-180 aksara. Satu idea sahaja.", sederhana: "220-350 aksara.", panjang: "380-500 aksara, untuk cerita penuh." };
  lines.push(`Panjang post: ${PANJANG[brief.panjang] || PANJANG.sederhana}`);
...
/** Prompt penuh (base + playbook + brief) — untuk disalin ke AI luar. */
export function buildPromptFor(brief, platforms = ["threads"], count = 3) {
  const playbooks = platforms.map(id => readPlaybook(id)).filter(Boolean).join("\n\n---\n\n");
  return [readBasePrompt(), playbooks, buildBrief(brief, platforms, count)].filter(Boolean).join("\n\n---\n\n");
}
...
export async function generate({ settings, brief, platforms, count }) {
  ...
  if (provider === "anthropic") {
    ...
    // Gateway mengehadkan output; minta secara jelas supaya post tak terpotong separuh jalan.
    // Sesetengah gateway menolak parameter yang model mereka tak sokong, jadi
    // temperature hanya dihantar bila pengguna memang menetapkannya.
    body = { model, max_tokens: 3000, messages: [{ role: "user", content: prompt }] };
    if (typeof temperature === "number") body.temperature = temperature;
  }
  // Gateway pihak ketiga kerap tersandung pada permintaan panjang: 502, 503, atau
  // tamat masa di hulu. Cuba beberapa kali sebelum menyerah, dan beritahu kenapa.
  const CUBAAN = 3, BOLEH_CUBA_LAGI = new Set([408, 409, 425, 429, 500, 502, 503, 504]);
  let res, json, raw = "", akhir = "";
  for (let cubaan = 1; cubaan <= CUBAAN; cubaan++) {
    try {
      res = await fetch(url, {
      ...
      if (res.ok) break;
      akhir = explain(res.status, json, raw);
      if (!BOLEH_CUBA_LAGI.has(res.status) || cubaan === CUBAAN) throw new Error(akhir);
    } catch (e) {
      const tamatMasa = e.name === "TimeoutError" || /aborted|timeout/i.test(e.message);
      akhir = tamatMasa ? "tamat masa menunggu pembekal" : (akhir || e.message);
      if (cubaan === CUBAAN) {
        throw new Error(`${akhir}. Dah cuba ${CUBAAN} kali. Cuba kurangkan bilangan post, atau tunggu sekejap — gateway tu mungkin sesak.`);
      }
    }
    await new Promise(r => setTimeout(r, cubaan * 4000));
  }

  const text = json?.content?.map(c => c.text).filter(Boolean).join("\n")
    || json?.choices?.[0]?.message?.content || "";
  let posts = parsePosts(text);
  if (!posts.length) {
    throw new Error("Model balas, tapi tiada post yang boleh dibaca dalamnya. "
      + `Permulaan balasan: ${String(text).replace(/\s+/g, " ").slice(0, 120) || "(kosong)"}`);
  }
  // Satu pusingan pembetulan: kalau model langgar peraturan sendiri, suruh dia tulis semula
  // dengan senarai masalah yang tepat. Lebih murah daripada pengguna membaca ayat buruk.
  const semak = posts.map((p, index) => ({ index, isu: lintPost(p, { sebutHarga: brief.sebutHarga === true, panjang: brief.panjang, angle: brief.angles?.[0] }) }));
  const arahan = arahanBetulkan(semak);
  if (arahan) {
    try {
      const body2 = { ...body, messages: [{ role: "user", content: `${prompt}\n\n---\n\n${arahan}\n\nPost asal:\n${text.slice(0, 4000)}` }] };
      const res2 = await fetch(url, { method: "POST", headers, body: JSON.stringify(body2), signal: AbortSignal.timeout(Number(process.env.LLM_TIMEOUT_MS || 180000)) });
      if (res2.ok) {
        const j2 = await res2.json().catch(() => null);
        const t2 = j2?.content?.map(c => c.text).filter(Boolean).join("\n") || j2?.choices?.[0]?.message?.content || "";
        const baru = parsePosts(t2);
        if (baru.length) {
          const isuBaru = baru.reduce((n, p) => n + lintPost(p, { sebutHarga: brief.sebutHarga === true, panjang: brief.panjang, angle: brief.angles?.[0] }).length, 0);
          const isuLama = semak.reduce((n, h) => n + h.isu.length, 0);
          if (isuBaru < isuLama) posts = baru;
        }
      }
    } catch { /* kekalkan yang asal kalau pembetulan gagal */ }
  }

  const lint = posts.map(p => lintPost(p, { sebutHarga: brief.sebutHarga === true, panjang: brief.panjang, angle: brief.angles?.[0] }));
  return { posts, provider, prompt, raw: text, lint };
}
...
function localPosts(brief, platforms, n) {
  ...
  const threadShapes = [
    { needsPrice: true, perlu: () => ada(hargaAda) && kel.length,
    ...
    { perlu: () => kel.length,
      make: () => ({ angle: "Aku silap", hook: "Aku beli yang mahal dulu. Rupanya tak payah.",
        body: baris(`${nama} buat kerja yang sama je.`, kel[0] + "."),
        reply: linkReply("Ada yang tanya link — ni dia:") }) },
    { needsPrice: true, perlu: () => ada(hargaAda),
    ...
    { perlu: () => true,
      make: () => ({ angle: "Soalan jujur", hook: `Korang yang dah guna ${nama} — memang tahan lama ke?`,
        body: baris(kel[0] ? `Setakat ni yang aku suka: ${kel[0].toLowerCase()}.` : "Aku tengah timbang nak ambil.",
                    "Weh aku nak dengar dari orang yang dah guna lama."),
    ...
    { perlu: () => kel.length >= 2,
      make: () => ({ angle: "Lawan keberatan", hook: '"Mesti susah nak guna." Aku pun fikir macam tu.',
        body: baris(kel[0] + ".", kel[1] + ".", "Takde hal pun, tak payah belajar lama."),
        reply: linkReply("Link:") }) },
  ];

  // Rangka berasaskan harga hanya dipakai bila brief membenarkan harga disebut.
  const bolehHarga = brief.sebutHarga === true;
  const usableThreadShapes = threadShapes
    .filter(sh => sh.perlu() && (bolehHarga || !sh.needsPrice))
    .map(sh => sh.make);
...
/**
 * Kandungan nilai: tips dalam niche, tanpa produk dan tanpa link.
 * Ini yang menjadikan akaun bukan kedai — dan playbook memang menuntut nisbah begini.
 */
export async function generateTips({ settings, niche, count = 2, panjang = "sederhana", audience = "", fetchImpl = fetch }) {
  const n = Math.max(1, Math.min(10, count));
  const brief = [
    "## Brief",
    "Jenis: kandungan nilai. TIADA produk, TIADA link, TIADA jualan langsung.",
    `Niche: ${niche || "[NICHE]"}`,
    audience ? `Audience: ${audience}` : "",
    "Tujuan: bagi satu benda berguna yang orang boleh guna hari ni juga, atau satu pemerhatian yang buat orang rasa difahami.",
    "Setiap post mesti berdiri sendiri, dan mesti membuka ruang untuk orang balas dengan pengalaman mereka.",
    "",
    "## Tugas",
    `Hasilkan ${n} post Threads. Panjang: ${panjang}.`,
    "Jangan sebut sebarang produk atau jenama. Jangan letak link. Jangan ajak beli apa-apa.",
    "Balas JSON sahaja mengikut kontrak output dalam arahan gaya.",
  ].filter(Boolean).join("\n");

  const prompt = [readBasePrompt(), readPlaybook("threads"), brief].filter(Boolean).join("\n\n---\n\n");
  const provider = settings?.llm?.provider || "local";

  if (provider === "local") {
    return {
      posts: Array.from({ length: n }, (_, i) => ({
        platform: "threads", angle: "Tips",
        caption: `[TIPS ${i + 1} PASAL ${String(niche || "NICHE").toUpperCase()} — isi sendiri, atau pasang AI untuk jana]`,
        reply: "", visual: "", hashtags: [], broll: [],
      })),
  ...
  if (anthropic) body.max_tokens = 3000;
  if (typeof temperature === "number") body.temperature = temperature;

  const res = await fetchImpl(url, { method: "POST", headers, body: JSON.stringify(body), signal: AbortSignal.timeout(Number(process.env.LLM_TIMEOUT_MS || 180000)) });
  const json = await res.json().catch(() => null);
  if (!res.ok) throw new Error(explain(res.status, json));
  const text = json?.content?.map(c => c.text).filter(Boolean).join("\n") || json?.choices?.[0]?.message?.content || "";
  const posts = parsePosts(text);
  if (!posts.length) throw new Error("Model tak pulangkan tips yang boleh dibaca.");
  return { posts, provider, prompt, lint: posts.map(p => lintPost(p, { panjang })) };
}
