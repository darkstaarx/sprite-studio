...
/** Slot ikut waktu pilihan pengguna (contoh 07:00, 13:00, 19:00), bermula dari tarikh diberi. */
export function customSlots(store, count, times, startDate) {
  const list = (times?.length ? times : ["07:00", "13:00", "19:00"]).slice().sort();
  const taken = new Set(store.data.posts
    .filter(p => ["scheduled", "review", "publishing"].includes(p.status) && p.scheduledAt)
    .map(p => p.scheduledAt));
  const base = startDate ? new Date(startDate + "T00:00:00") : new Date();
  const out = [];
  for (let d = 0; d < 60 && out.length < count; d++) {
    for (const hhmm of list) {
      ...
      const ts = t.getTime();
      if (ts <= Date.now() || taken.has(ts)) continue;
      taken.add(ts);
      out.push(ts);
      if (out.length >= count) break;
    }
